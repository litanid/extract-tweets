...
for month_tweets_filename in sys.argv[1:]:
	...
	with open(outputFile_name,'w') as outputFile:
		print("<h4><strong>本次归档共计 {0} 条推文：</strong></h4>\n".format(len(data_tweets)),file = outputFile)
		...
		for tweet in data_tweets :
				...
				for media in medialist:
					if media['type']=="photo" :
						...
						if os.path.isfile(local_media_filename):
							...
							print("<a href=\"{0}\"><img class=\"alignnone size-medium\" src=\"{1}\" /></a>\n".format(local_media_filename,local_media_filename),file = outputFile)
						...
						if os.path.isfile(local_media_video_filename):
							...
							print("[videos href=\"{0}\"]{1}[/videos]\n".format(local_media_video_filename,local_media_image_filename),file = outputFile)
						else:
			...
			print("<a href=\"{0}\" target=\"_blank\">{1}</a>\n".format(tweetlink_url,local_dt),file = outputFile)
			print("<hr />\n",file = outputFile)
			print("---------------------------------------")
	print("\n##############")
